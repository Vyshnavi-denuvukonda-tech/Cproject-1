# Cproject-1
Here, It is a Mini project in C for managing student records using file handling. Allows Adding, Viewing, Updating , Searching and Deleting student data
# C Project: Student Record Management System

This is a mini project in C for managing student records using file handling.  
It allows users to add, display, search, update, and delete student records through a simple menu-driven interface.

## Features
- Add new student records  
- Display all existing records  
- Search student records by ID number  
- Update student information  
- Delete student records  

## Technologies Used
- C Programming  
- File Handling  

## How to Run
1. Compile the program using any C compiler (e.g., GCC or Code::Blocks).  
2. Run the executable file.  
3. Use the on-screen menu to perform operations.

## Author
Developed by Vyshnavi Denumukonda
